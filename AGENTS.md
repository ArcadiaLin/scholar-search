...
- **`docs/*.md`** 是**设计**——概念、契约、约束、论证。相对稳定。
- **`docs/develop/`** 是**推进计划**——做什么、什么顺序、怎么验、什么是坏的。
  随实现推进不断变化。

**要动手做事，从 `docs/develop/README.md` 开始。** 那里有当前位置、下一步、
以及本目录五份文档各自回答什么问题。

### 11.1 设计层的阅读顺序

**层内可以跳读，跨层不要跳**：上层是下层的前提，绕过上层直接实现下层，
会把已经论证过的约束当成可以自由选择的设计。
...
| | `skill-decomposition.md` | 94 条检索指导的逐条归属（`TOOL`/`SVC`/`HP`/`CAP`/`NPa`/`NPj`/…） | 要新增工具、或要写偏好条目之前 |
| | `metascientist-rerank-design.md` | 从 CiteFlow 提炼的 rerank 算法设计 | 只在做排序算法时 |
| | `reviewer-design.md` | Sidecar Reviewer 第一版的完整设计（证据、检测器、边界判据、五条决策） | 做 S11 之前必读 |

### 11.2 推进计划层

| 文档 | 回答什么问题 |
| --- | --- |
| `develop/README.md` | 现在在哪？下一步是什么？**入口** |
...
（三条前置修复）      （答案池）  （Reviewer）  （判别层）
```

三条前置修复不是 stage，是 S10 的前提：在 L0 召回层系统性漏答案的前提下，
后面每个 stage 的产出都不可信（理由见 `develop/plan.md` §2）。

### 11.3 两条硬纪律

这两条是路线上反复被违反、且每次都有代价的，单独拎出来：

1. **判据先于实现，且不得事后放宽**（`develop/plan.md` §8）。
   实现完成后发现判据没满足，把差额写进 `develop/backlog.md` 的"验收缺口"，
   不要改判据去迁就实现。`DONE†` 这个标记就是为记录这种差额而存在的。
2. **路线没规定而实施中做出的选择，记成 `D-nn` 决策**，
   写进 `develop/decisions.md`。理由、代价、被否决的替代方案三项都要写。
