...
**接手实施前先读 §11 的文档地图。** 本文件只规定"怎么做"，不记录"现在做到哪、下一步做什么"——
那些在 `docs/` 下，且有固定的阅读顺序。跳过 §11 直接改代码，几乎一定会重做已经论证过的决策。
...
`docs/` 分四层。**层内可以跳读，跨层不要跳**：上层是下层的前提，
绕过上层直接实现下层，会把已经论证过的约束当成可以自由选择的设计。

### 11.1 阅读顺序

| 层 | 文档 | 是什么 | 什么时候读 |
| --- | --- | --- | --- |
| 研究 | `agentic_search_preference_reviewer_research_design.md` | 研究理念与文献综合 | 想知道"为什么做这个" |
| 形式化 | `design.md` | 模块、契约、约束的行为抽象；两个时间尺度、四个模块、$NP/HP/PH$ | **动任何架构之前必读** |
| 规格 | `prototype.md` | 可实现的具体方案：工具集、排序栈 L0–L3、评价协议 | 实现检索或排序之前 |
| | `search-service.md` | Service 的接口、机制与边界 | 改 `src/search-service/` 之前 |
| | `experiments.md` | 待检验的命题与判据：B/E/J/P/M 五个消融轴 | 动任何影响实验对照的东西之前 |
| | `metascientist-rerank-design.md` | 从 CiteFlow 提炼的 rerank 算法设计 | 只在做排序算法时 |
| | `05-skill-decomposition.md` | 94 条检索指导的逐条归属（`TOOL`/`SVC`/`HP`/`CAP`/`NPa`/`NPj`/…） | 要新增工具、或要写偏好条目之前 |
| 执行 | `06-widi-scholar-roadmap.md` | S0–S9 的 stage 定义（**只读**），以及 §3"关于验收"的纪律 | 接手实施必读，尤其 §3 |
| | `06-progress.md` | 这条路线的**状态来源**：已完成、验收缺口 G-1..G-5、决策记录 D-nn | 接手实施必读 |
| | `07-widi-mapping.md` | 设计概念 → 本仓库实际位置的对照表 | 要找某个概念的代码在哪 |
| 当前 | `08-retrieval-defects.md` | F-1..F-11，真实会话追出的缺陷，每条带可复现证据 | **下一步要修的东西** |
| | `09-next-stages.md` | S10（结构化答案池与召回回路）、S12（判别层） | 同上 |
| | `10-reviewer-v0.md` | S11（Reviewer v0 与 $NP_0$ 重写） | 同上 |

### 11.2 当前推进路径

```
F-1 / F-2 / F-10  →  S10  →  S11  →  S12
（08 的三条前置修复）   （09）   （10）   （09）
```

三条前置修复不是 stage，是 S10 的前提：在 L0 召回层系统性漏答案的前提下，
后面每个 stage 的产出都不可信（理由见 `09-next-stages.md` §1.1）。

### 11.3 两条硬纪律

这两条是路线上反复被违反、且每次都有代价的，单独拎出来：

1. **判据先于实现，且不得事后放宽**（`06-widi-scholar-roadmap.md` §3）。
   实现完成后发现判据没满足，把差额写进 `06-progress.md` 的"验收缺口"，
   不要改判据去迁就实现。`DONE†` 这个标记就是为记录这种差额而存在的。
2. **路线图没规定而实施中做出的选择，记成 `D-nn` 决策**（同上 §4），
   写进 `06-progress.md`。理由、代价、被否决的替代方案三项都要写。
