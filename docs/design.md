...
**检索失败时的处置：显式上报并降级，不向外求助。** 反复重写仍无有效召回时，
Agent 必须产出带失败说明的 $SO$——标注哪些子查询失败、失败分类、已覆盖与未覆盖的方向——
而不是继续消耗预算，也不是静默返回空结果。三条约束：

- **不得向用户追问**。评测模式下没有可追问的对象，而"有人可问"与"无人可问"两种行为
  会使同一套 $NP_k^{agent}$ 在评测与真实使用中产生不同轨迹，破坏可比性；
- **不得转为向 Reviewer 求助**。Reviewer 是旁路观察者，不是被调用方（§5.2）。
  让 Agent 主动拉取建议会把 $A_t$ 的触发权从固定策略移交给 Agent，
  使介入率成为内生变量，$\Delta_{\mathrm{sidecar}}$ 的归因随之失效；
- **降级结果仍须满足 §2.4 的输出契约**，失败信息是结构化字段而不是自然语言致歉。

部分结果加显式缺口说明，比空结果或超预算的穷尽搜索都更有用，
也让 Reviewer 的离线归因能区分"策略不对"与"该查询在当前数据源下不可解"。
...
### 4.1 Evidence Store：episode 作用域的证据积累

跨工具调用积累候选与证据是检索的固有需求：$t$ 步召回的论文要在 $t+3$ 步参与引文扩展与排序。
承载它的 **Evidence Store 是 episode 作用域的运行时状态**，归属由三条界定：

- **由 Service 持有**，与 `RunSnapshot` 同生共死，episode 结束即销毁；
- **不是 Service 的跨 episode 记忆**。§4 说"Service 不持有独立状态"指的是**跨调用决策状态**，
  不是禁止在一次 episode 内累积证据。判据仍是 §2.2：
  Service 的每次调用由显式输入加当前 episode 的证据集合构成充分统计量，
  不携带**跨 episode** 记忆，不做动作选择。跨 episode 的记忆只有一个合法载体，即 $PH_k$；
- **不是 Agent 的私有上下文**。它的统计量与结构化诊断必须进入 $\bar{\tau}_t$（§5.1），
  否则 Reviewer 无法判断覆盖缺口——那正是它最主要的审查对象。

一个实际后果：**Agent 不通过上下文携带候选集**。工具返回的是候选的引用与摘要视图，
完整证据留在 Store 内，由后续工具调用按 id 引用。这既是上下文成本问题，
也是可观察性问题——留在 Agent 上下文里的候选集不进 $\bar{\tau}_t$。

Store 的内容契约、字段与生命周期见 `search-service.md` §5.3。
...
### 6.1 代表性示例属于偏好，不属于提示词

$NP_k$ 的条目有两种形态：**陈述式**（"先覆盖各个方向，再深入"）与**示例式**
（一个查询分解或一次判别的具体演示）。两者都是自然语言、都由语言模型消费，
因此**都在 $PH_k$ 内**，都随 $k$ 版本化，都可被 `propose_preference_update` 改写。

把示例固定在 $SP$ 里是常见做法，但在本架构下不成立：示例往往比陈述更强地决定模型行为，
若它不在 $PH_k$ 内，$k$ 尺度就对系统最有影响力的那部分输入没有作用面，
消融也无法回答"学到的偏好能否达到人工撰写的水平"——因为两个实验组共享同一批示例。

因此约束与陈述式条目相同：可独立引用、可独立改写、可逐条关闭、回放可定位到具体版本。
一条额外要求是**示例必须携带来源**：取自真实轨迹的哪个 episode、哪次调用、
或标注为人工构造。没有来源的示例无法在归因时判断它是否已被后续证据推翻。

$NP^{\mathrm{judge}}$ 侧同理：判别准则里的正反例与准则文本同属一个版本化对象。
