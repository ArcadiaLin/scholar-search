...
### 2026-08-22 — 三次会话的横向审阅，得到一个负结果（不是 stage）

`runs/` 下现在有三次 search 会话，其中 `98ar` → `k9u1` 构成一次
**手工执行的 P 轴消融**：同一条 `AutoScholarQuery_train_1`、无上下文的新 agent、
唯一变量是 `98ar` 自己提炼的四条通用提示。

```
search-d9w6  无提示            1/4
search-98ar  无提示            0/4（拿到 gold 后靠标题查询补齐）
search-k9u1  带四条提示        0/4，第二条查询 1/5
```

**提示没有帮助。** 逐条核对，四条里三条完全没有被执行——包括 agent
在开场明确复述过的那一条（"我会同时尝试完整短语和关键词组合查询"，
随后 30 条查询无一条是短语查询）。

工具使用统计：

```
d9w6  40 次 6/9 种    98ar  35 次 3/9 种    k9u1  64 次 4/9 种
三次加起来 facet_probe / rank_candidates / search_fulltext 零调用
```

**三个诊断型工具全部闲置，agent 只用生成型工具（搜 → 取 → 搜 → 取）。**
而且带提示的那次调用量几乎翻倍、工具种类反而更少——压力让它做更多的同一件事。

由此排出 **S11（Reviewer v0 与 $NP_0$ 重写）**，定义与全部证据在
`docs/10-reviewer-v0.md`；原 S11（判别层）顺延为 **S12**。
核心设计原则是**每条 $NP$ 条目必须绑定一个可从 $\bar{\tau}_t$ 读出的观察量**——
不可观察的条目产出的是复述而不是执行。

同时新增两个缺陷：**F-10**（`expand_citations` 不接受其他工具产出的 id，
一次失败后 agent 彻底放弃了引文扩展这条路）与 **F-11**（OpenAlex 对 ML 预印本的
引文图很稀疏，抽样 8 篇 gold 有 4 篇 `referenced_works` 为空、1 篇无记录，
backward 扩展基本不可用，E 轴的默认方向要改成 forward）。
