...
> `06-progress.md`（状态与验收缺口 G-1..G-5）、`08-retrieval-defects.md`（F-1..F-9）
...
2. 下面两个 stage 的**理由比定义本身长**。roadmap 的 stage 条目是刻意精简的，
   而 S10 / S11 各自涉及一个需要论证的架构选择，塞进那个格式会把论证挤掉。
...
所以 S11 的验收判据（§3.4）不应该是"`/judge` 能返回 200"。

---

## 5. 本文范围之外

这几件事被讨论过但**不进 S10/S11**，记下来免得后面重新捡起：
...
- **G-1 Reviewer 时机**：S10 给它造出了作用面（§2.1 第三条），
  ...
  checkpoint 判定移到工具执行边界是另一个 stage。
