...
### 3.4 provider 的三种角色与降级链

能力表回答"某个源能做什么"，但不回答"同一能力有多个源时先用谁"。
后者由**角色**与**降级链**决定，二者都是配置，不是代码分支。

三种角色，按能力分别指派（同一个 provider 在不同能力上可以担任不同角色）：

| 角色 | 含义 | 失败时 |
| --- | --- | --- |
| **主源** | 该能力的默认承担者，也是溯源与统计的基准 | 触发降级链 |
| **补充源** | 主源覆盖不全时按需补齐，结果与主源合并 | 记为部分缺失，不阻断 |
| **兜底源** | 前两者都拿不到时的最后手段，通常成本或配额受限 | 标注能力缺失 |

**降级链是有序的能力级配置**，而非源级：`graph.citations` 的链与
`text.abstract` 的链可以完全不同。触发条件必须是可判定的谓词——
返回为空、覆盖率低于阈值、限流、鉴权失败——而不是"结果看起来不好"这类判断。
每一跳都要记录：触发原因、耗时、是否补齐，并进入可观察状态。

三条约束：

1. **降级不改变字段路由的优先级**。补充源补进来的字段仍按 §5.2 的路由表定值，
   不因为"它是最后拿到的"就覆盖主源；
2. **部分结果优于阻断**。链条走完仍缺失时置缺失指示位并继续，不报错、不静默填零；
3. **降级次数计入预算**。一次逻辑调用可能产生多次网络调用，
   成本模型按实际发生计账，否则预算前沿的横轴不可信。

降级链的存在使"接入一个新 provider"这件事保持在配置层：
新源注册能力表与成本模型后，只需在相关能力的链上插入一个位置。**[设计]**
...
### 5.3 Evidence Store：内容契约与生命周期

管线各阶段共享一个 **episode 作用域的证据集合**。上游契约见 `design.md` §4.1：
由 Service 持有、与 `RunSnapshot` 同生共死、其统计量进入可观察状态、
Agent 按 id 引用而不在上下文里搬运候选集。本节定义它装什么。

**内容**分四类：

- **候选记录**：统一 Paper schema 的实例，含字段级溯源与质量标记（§5.1）；
- **发现溯源**：每条记录**因何进入集合**——哪次调用、哪个子查询、
  经由检索还是引文扩展、扩展自哪个种子。这一项决定 Reviewer 能否归因，
  也决定"候选覆盖与最终排序分开报告"这条评价要求能否兑现；
- **派生信号**：特征值、各级打分、判别结果与它们的版本号（`feature_version`、
  准则版本、模型与校准版本），使同一 episode 内的分数可比；
- **过程账目**：各阶段的调用数、耗时、缓存命中、失败分类与预算消耗。

**生命周期**：`RunSnapshot` 建立时创建，episode 终止（正常结束、预算耗尽或失败降级）时销毁。
只有三样东西允许离开 episode：写入回放缓存的原始响应与特征矩阵（§7.7 的训练输入）、
归档的 $\bar{\tau}_t$、以及最终的 $SO$。**其余一律不跨 episode**——
跨 episode 的记忆只有 $PH_k$ 一个合法载体。

**一条必须守住的边界**：Store 里可以有打分与判别结果，但不能有"下一步做什么"。
出现被持久化的策略状态（待办队列、未完成的扩展计划、跨调用的重试意图），
Service 就成了第二个决策主体，§1 的职责边界随之失效。判据见 §7.2。
