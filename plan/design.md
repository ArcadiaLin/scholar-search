...
Main Search Agent 的每一轮都遵循“思考/规划 -> 工具调用 -> 观察 -> 更新上下文”的过程。
Reviewer 的建议作为额外的、带来源标记的 advice 注入下一轮，而不是直接篡改 Agent
已有轨迹：
...
其中预算和最大迭代次数由运行时强制执行，不能只依赖模型自行遵守。

## 5. Reviewer Agent：旁路审查与反馈闭环

Reviewer Agent 也是一个受控的 ReActLoop：

```text
Reviewer Agent = ReActLoop(ReviewContext + Review Tools)
ReviewContext_0 = ReviewerSystemPrompt + ReviewInput
```

`ReviewInput` 有两个来源：

1. **过程审查输入**：Main Search Agent 的上下文、工具调用、候选集合和中间观察值；
2. **结果反馈输入**：`SO` 与 `GoldenAnswer`（评测场景）或 `UserFeedback`（真实使用场景）。

Reviewer 有两个互补但边界不同的功能。

### 5.1 Provide Advice（过程建议）

```text
PA(trajectory, reviewer_context) -> Advice
```

Reviewer 检查查询是否覆盖关键约束、检索结果是否存在明显噪声、是否需要补充来源或
引文扩展、当前策略是否接近预算上限，并输出结构化 `Advice`，例如：
...
Advice 只通过事件/消息通道交给 Main Search Agent，作用于下一轮决策；Reviewer 不直接
改写 Main Agent 的上下文，也不绕过 Search Service 调用未预算的 API。

### 5.2 Compare Answer（结果比较）

```text
CA(SO, GoldenAnswer | UserFeedback) -> PreferenceUpdate
```

该过程用于分析漏召回、误召回、排序偏差、展示偏好和关系图缺失等问题。Reviewer 通过
`update_preference` 工具向 Preference Persistence 提交带理由、版本和置信度的更新，
而不是直接写入存储。
...
+----------+-----------+       +----------+-----------+
  ...
  Main trajectory / SO ----> Reviewer Agent ----> Advice -> Main next turn
                                  |
                                  +---- PreferenceUpdate -> Preference Store
```
...
Main Agent -> Search Service -> candidates/observations
Main trajectory -> Reviewer -> Advice -> next Main turn
SO + feedback/golden -> Reviewer -> PreferenceUpdate -> PH_(next run)
```

## 8. 边界、可观测性与实现原则

1. **Agent 与 Service 解耦**：模型只决定检索策略和参数；网络访问、去重、排序基础算子、
   预算检查和记账由 Service/运行时负责。
2. **Reviewer 是旁路控制器，不是第二个主搜索器**：它默认不独立扩大检索面，只对主循环
   提供可解释建议；需要检索证据时也必须经受控工具。
3. **偏好不是隐式记忆**：所有偏好都要有 schema、版本、来源、置信度和生效范围。
4. **结果不是纯文本**：论文列表、证据、关系图、失败项和运行元数据都应能机器读取。
5. **成本是控制流的一部分**：在运行前估计并在运行中强制限制模型调用、API 调用、
   并发、候选规模和墙钟时间；缓存命中与冷启动分别统计。
6. **评测可复现**：记录 query ID、`end_date`、偏好版本、模型/Provider 版本、预算、
   原始事件和最终 `SO`，使 Precision、Recall、F1、延迟、调用数和 Token 可复核。

该架构保留原设想中的四个核心实体，同时明确了数据契约、Reviewer 的两个时机、
Preference 的生效边界，以及 Search Service 内部的多源检索职责。